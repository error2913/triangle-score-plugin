...
引用结算截图上传成绩（人工审核）→ 比赛结束后用纯文本展示接下来的比赛。插件通过
WebSocket 实时接收控制器状态（自动重连），对接「三角占领 · 赛时控制器」的成绩上传协议。
...
- 群管理发送 `.ts start <对局ID>` 选择该场 → bot @ 全部选手并**直接开局**
  （开局前自动随机选边：调用后端 `POST /api/bot/matches/{id}/randomize-sides`
  交换掠夺者/守护者阵营，随后 `POST /api/init` 自动记录秘钥，不对外展示），发棋盘截图；
- 选手**引用（回复）结算截图** + 触发词（默认「上传成绩」）→ image-recognizer 识别 →
  bot 发【成绩待确认】消息 → **群管理**引用回复 确认 / 修改 / 拒绝 → 确认后才上传；
- 插件加载即连接控制器 `/ws` 状态通道（WebSocket，指数退避自动重连）；控制器后端
  在最后一分钟自动开始超时判定，超时后经 `/ws` 推送 `game_over`，插件自动结束对局
  并用纯文本展示接下来的比赛（最多 4 场 + 余量提示）；`.ts stop` 同样展示；
- `.ts reconnect` 可手动重连控制器 WS（断线连续 3 次会在对局群提示，恢复后自动继续）；
...
- 赛时控制器服务（`competition_web/demo`，需已实现 `/api/v1/results` 协议与
  `/api/init`、`/api/end`、`/api/state`、`/ws` 状态通道）；
...
3. 在插件设置中填入：
   ...
   - `siteToken`：比赛网站机器人令牌（对应后端 `.env` 的 `BOT_API_TOKEN`），
     `.ts start` 开局前自动随机选边时使用；后端未配置或留空则跳过自动选边；
   ...
   - `debugLog`：调试日志开关（默认 `1`）。截图/WS 状态详情会打印到海豹日志，
     排查「截图失败」「比赛不结束」「WS 断线」时保持开启，稳定后可以改 `0`；
...
.ts reconnect 手动重连控制器 WebSocket（仅群管理以上）
...
3. bot **自动随机选边**（后端 `BOT_API_TOKEN` + 插件 `siteToken`；未配置时跳过并提示），
   随后 @ 已填 QQ 的选手并**直接开局**（`POST /api/init` 自动记录秘钥、发送棋盘截图）；
   未填 QQ 的一侧会收到警告，仍可强制开局，但该侧结算截图无法自动匹配身份上传；
4. 比赛进行中，群管理可 `引用结算截图 + 上传成绩`；
5. 比赛时间到：控制器后台在最后一分钟自动开始超时判定，超时后经 `/ws` 推送
   `game_over`，插件自动结束对局并用纯文本展示接下来的比赛
...
> 开局提示：选手未在网站个人资料填写 QQ 时开局不拦截，仅警告（无法 @ 与自动匹配身份，
> 需选手补填后重新开局）。控制器 WS 若连续 3 次重连失败，bot 会在对局群提示控制器
> 可能失联；恢复后自动重连并清零失败计数（也可用 `.ts reconnect` 手动重连）。

## 插件内部机制（重载安全）

- 状态通道是 WebSocket（`new WebSocket`，海豹 1.5.1+ 提供），不依赖常驻轮询定时器：
  插件加载（onLoad）即尝试连接控制器 `/ws`，连接句柄与重连状态挂在全局
  （`globalThis.__tsWsState`），断线用单次 `setTimeout` 指数退避重连
  （1s → 2s → 4s … 封顶 30s），不存在会被重载打断的常驻循环；
- **热重载不打断进行中的比赛**：对局身份、秘钥、待审成绩全部存在 `ext.storage`，
  重载后 onLoad 幂等地确认 WS 连接，海豹已关闭的旧连接会被自动重建，
  正在进行的对局照常上传、审核和结束；插件加载时会在海豹日志打印恢复信息
  （`检测到进行中对局…控制器 WS：已连接/重连中`），可据此确认重载后比赛已接续；
- 待审成绩 15 分钟未审自动作废（在 WS 状态更新 / 上传交互时惰性清理）。
...
- 状态通道（控制器侧）：`WS {controllerUrl}/ws`。连接后立即收到一帧
  `{"type":"state_update", ...完整状态}`，之后每次状态变更（含超时 `game_over:true`）
  都会推送；超时判定由控制器后端后台任务驱动，插件不再调用 `/api/tick` 轮询；
...
5. **控制器需重启**：升级到带 `/ws` 状态通道 + 后台超时看门狗的版本后，需要
   **重启控制器进程**（不是热更新）才生效；重启后用 `WS {controllerUrl}/ws`
   连接测试，能收到 `state_update` 帧即正常。
